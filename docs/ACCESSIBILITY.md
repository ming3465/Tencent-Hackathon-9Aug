...
The Hawker Centre `ENTRY` must start closed with its blocker enabled, use the
same horizontal slide-collapse animation as the original DoorView, and return
to full-size closed leaves after exiting. Every reusable returned door must
likewise report its closed 0° pose and 1×/1× leaf scale.
