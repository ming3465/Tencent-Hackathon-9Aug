...
| P2-11 | **Audio fixes**: stop the ambience `setInterval` on `visibilitychange` (queued oscillators currently all fire at once on tab return), stop firing the 440Hz UI blip on every dialogue line (~24 per playthrough), wire up or delete the unused `interact` event, add voice-leading to the pad, reduce the global 230ms/0.26 slapback on UI | A burst of stacked oscillators on tab-return during a live demo is a memorable failure | 2h | Low |
