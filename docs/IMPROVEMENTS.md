...
| **Cohesive title/VN/audio presentation pass** — a reviewed OpenAI HDB-estate panorama replaces the mismatched title illustration after a documented pseudo-writing rejection and constrained cleanup; all interface copy stays semantic HTML; named portraits gain neutral/thoughtful/warm expressions; a sole accessible `>` replaces the full-width Continue bar; the ambient pad gains deterministic four-chord day/evening voice-leading; interaction labels use tactile Canvas-safe code-drawn plates; room walls/floors gain scale, material, contact shade, and daylight; the chapter-light glaze is restrained to preserve the palette; and retired title-illustration CSS no longer ships | 2026-08-04 |
...
| **Architectural projection-depth pass** — all eight exterior structures now share audited shallow three-quarter geometry with hipped/sawtooth roof planes, side faces, contact shadows, and seven recessed entries; the existing doors, collision gaps, occlusion crops, and travel routes remain aligned | 2026-08-04 |
...
| P2-11 (complete) | **Audio polish:** visibility suspends/resumes the audio context; dialogue advances no longer replay the 440 Hz open blip; `interact` is wired; UI tones bypass reverb; the global send is reduced; surface footsteps reuse one buffer; and deterministic four-chord day/evening progressions now use shared-tone voice-leading | The live-demo burst, repetitive UI, and unrelated-random-note risks are closed | Done | Low |
...
| P2-17 (complete) | **Unify the title screen and playable art direction.** A reviewed HDB-estate panorama now bridges the code-drawn palette and storybook key art; its reject/edit/optimization trail is preserved, its crop is responsive, and all title/interface text remains semantic HTML | The first impression now promises the same Singapore, dignity, palette, and visual-novel warmth the game delivers | Done | Low |
...
| P3-4 (complete) | **Commit to one projection** — all eight large structures now resolve one shallow three-quarter geometry registry with roof planes, side faces, contact shadows, and recessed thresholds. Trees retain grounded trunks and offset crowns; no isometric-game asset or layout was copied | Done |
