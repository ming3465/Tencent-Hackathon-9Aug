...
**7. Density + coherence pass on the map. (1 day)**
...
What: `npx edgeone pages deploy dist -n kampung-sg -t <token>`; then `node scripts/browser-smoke.mjs --url <edgeone-url>` and screenshot 21 passes.
...
**Q&A prep — acknowledge, never defend:** Playtesting → "Five sessions with real older adults; here's what we changed." Pacing → "A full loop is one cup of kopi long — deliberate for this audience; the demo path is 3 minutes for yours." No runtime LLM → deliver the §5 line. No timer/failure → "Deliberate: Oxford's Animal Crossing findings and the Gerontoludic Manifesto — growth over decline."
...
**"An AI companion that can hallucinate, lag, or leak at a lonely senior isn't social good — so we put the AI where the rules ask for it, in creation: it built our world, our characters, and our art, and what you just played ran flawlessly on stage Wi-Fi because there is nothing left to fail."**
...
11. **The "elderly care is a named sub-theme" finding was never operationalized.** 适老化关怀 is literally a listed Little Red Flower/Social Good sub-theme. One deck line — "our theme is a named sub-theme of the official Social Good track" — is the cheapest legitimacy claim available and appears in no action list. Same for the one-hour anti-scam dialogue beat (touches "game security systems," a named qualifying AI area, and rhymes with the May winner) — proposed in Memo B, costed by nobody.
