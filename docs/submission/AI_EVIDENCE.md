...
|---|---|---|---|
| **Code** | Phase 1 graybox (10 files, incl. the shipping `matchEngine.ts`) by **CodeBuddy CLI 2.127.0**; sandbox scene, state and UI by **OpenCode**; audio system, resident behaviour, lighting and the smoke harness by **Claude Code (Opus 5)** — all under human direction | Architecture rule: domain logic stays Phaser-free and pure; Phaser lazy-loaded so the title page ships small; no backend, no network | `tsc --noEmit` clean (strict) · `vitest` 59/59 pass · `vite build` succeeds — 24.35 kB initial JS (8.85 kB gzip), Phaser deferred to a lazy 1,498 kB chunk (345.64 kB gzip) · `npm audit` 0 vulnerabilities · `npm run smoke` — a Node + Chrome DevTools Protocol harness (`scripts/browser-smoke.mjs`) driving the **production** bundle: **19 checks, 19 passing, 0 failing, run 2026-08-01** |
...
| Browser smoke harness | `scripts/browser-smoke.mjs` · `npm run smoke` | Node + Chrome DevTools Protocol, driving the **production** bundle: 19 checks, 19 passing, run 2026-08-01. Runs on macOS and Linux. (`scripts/browser-smoke.ps1` is **superseded** — retained as the pre-pivot harness for the card-matching build, and is not evidence for the current game) |
...
*Verified 2026-08-01 on the current build: `npm run typecheck` clean (strict) · `npm test` 59/59 passing across 3 files (31 match engine · 11 sandbox state · 17 audio) · `npm run build` succeeds · `npm audit` 0 vulnerabilities · `npm run smoke` 19/19 checks passing against the production bundle. (`docs/AI_USAGE_LOG.md` records 42 tests at the sandbox-pivot entry on 2026-07-25; the 17 audio-settings tests were added on 2026-08-01, which is what moves the count to 59.)*
