...
| **What survived** | The match engine CodeBuddy wrote **still ships today** as the optional memory-table activity — 31 of our 65 tests cover it. |
...
|---|---|---|---|
| **Code** | Phase 1 graybox (10 files, incl. the shipping `matchEngine.ts`) by **CodeBuddy CLI 2.127.0**; sandbox scene, state and UI by **OpenCode**; audio system, resident behaviour, lighting and the smoke harness by **Claude Code (Opus 5)** — all under human direction | Architecture rule: domain logic stays Phaser-free and pure; Phaser lazy-loaded so the title page ships small; no backend, no network | `tsc --noEmit` clean (strict) · `vitest` 65/65 pass · `vite build` succeeds — 24.35 kB initial JS (8.85 kB gzip), Phaser deferred to a lazy 1,498 kB chunk (345.64 kB gzip) · `npm audit` 0 vulnerabilities · `npm run smoke` — a Node + Chrome DevTools Protocol harness (`scripts/browser-smoke.mjs`) driving the **production** bundle: **21 checks, 21 passing, 0 failing, run 2026-08-01** |
...
| **Tests** | Test bodies drafted by AI — match-engine tests by CodeBuddy CLI 2.127.0 (rewritten by OpenCode after they failed the gate), sandbox-state tests by OpenCode, audio tests by Claude Code (Opus 5) | Rule: **tests are the acceptance criteria, and the agent that writes the code does not get to declare it passing** | 65 deterministic tests across 3 files (31 match engine · 17 sandbox state · 17 audio settings), all pure — no browser, no timers, no network. Re-run in ~1 second: `npm test` |
...
| Browser smoke harness | `scripts/browser-smoke.mjs` · `npm run smoke` | Node + Chrome DevTools Protocol, driving the **production** bundle: 21 checks, 21 passing, run 2026-08-01. Runs on macOS and Linux. (`scripts/browser-smoke.ps1` is **superseded** — retained as the pre-pivot harness for the card-matching build, and is not evidence for the current game) |
| Claim guardrails | `docs/RESEARCH.md` | The prohibited-claims list, written before any copy existed |
| Test suite | `src/game/__tests__/` | 65 passing deterministic tests |
...
*Verified 2026-08-01 on the current build: `npm run typecheck` clean (strict) · `npm test` 65/65 passing across 3 files (31 match engine · 17 sandbox state · 17 audio) · `npm run build` succeeds · `npm audit` 0 vulnerabilities · `npm run smoke` 21/21 checks passing against the production bundle. (`docs/AI_USAGE_LOG.md` records 42 tests at the sandbox-pivot entry on 2026-07-25; the audio-settings and dialogue-script tests added on 2026-08-01 are what move the count to 65.)*
